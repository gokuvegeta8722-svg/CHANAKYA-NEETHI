...
class Level2(BaseLevel):
    ...
    def setup(self):
        print("\n[LEVEL 2] Bharukaccha - Dana (Reward/Trade)\n")
        try:
            ...
            merchant = NPC("merchant", NPC_DATA["merchant"]["name"], 300, 400, merchant_sprite, NPC_DATA["merchant"]["dialogue"])
            ...
            for i in range(4):
                x = 200 + i * 300
                y = 200
                enemy = Enemy(f"soldier_{i}", x, y, "soldier", soldier_sprite)
            ...
            map_piece = MapPiece(800, 250, icon_sprite)
            self.items.append(map_piece)
            
            print("[LEVEL 2] Ready!\n")
        except Exception as e:
            print(f"[ERROR] Level 2 setup failed: {e}")
            raise